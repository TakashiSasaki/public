...
def main():
    parser = argparse.ArgumentParser(
        description="Scan directory tree and output as JSON conforming to dirtree.json schema."
    )
    ...
    parser.add_argument("-p", "--progress", action="store_true", help="Show scan progress")
    ...
    tree = scan_directory_tree(args.directory, progress=args.progress)
    
    if args.output:
        save_to_json(tree, args.output)
        print(f"Tree saved to {args.output}")
    else:
        # Print to stdout (only if not showing progress, to avoid mixing output)
        if not args.progress:
            print(json.dumps(tree, ensure_ascii=False, indent=2))
        else:
            print("(Use -o to save output to file when using --progress)")
