...
<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
    ...
    {/* Cloud Silhouette */}
    <path
        d="M10,22 C6,22 4,19 4,16 C4,13.2 6.2,11 9,11 C10,7 13.5,5 17.5,6 C21.5,7 23,10 23,12 C26,12.5 28,15 28,18 C28,21 25.5,22 22,22 L10,22 Z" />
    {/* Inner Box (Instance) */}
...
<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
    ...
    <rect x="2" y="10" width="28" height="16" rx="1" className="fill-white dark:fill-gray-900" />
...
<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
    ...
    <path d="M9,18 h14 M9,21 h14 M9,24 h14 M9,27 h14" strokeWidth="1" strokeDasharray="2 2" />
</svg>
),
...
<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
    ...
    <path d="M14,13 h14 M14,16 h14 M14,19 h14" strokeWidth="1" strokeDasharray="1.5 2" />
</svg>
),
ComputeServer: (props) => (
<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
...
<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"
    ...
    {/* Workpiece */}
    <rect x="6" y="10" width="4" height="3" fill="currentColor" stroke="none" />
    {/* Table */}
...
const CompositeCursor = ({ baseName, badgeName, badge2Name, colorClass, size = 64 }) => {
...
return (
<svg width={size} height={size} viewBox="0 0 32 32" className={`overflow-visible ${colorClass} transition-colors
    ...
    <g transform="translate(-4, -4)">
        ...
        <circle cx="8" cy="8" r="8" className="fill-white dark:fill-gray-900" />
...
const [selectedColor, setSelectedColor] = useState('text-red-600 dark:text-red-400');

const colorOptions = [
{ label: 'Production (Red)', value: 'text-red-600 dark:text-red-400', bg: 'bg-red-100 dark:bg-red-900/30' },
{ label: 'Development (Green)', value: 'text-green-600 dark:text-green-400', bg: 'bg-green-100 dark:bg-green-900/30' },
{ label: 'Normal (Blue)', value: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-100 dark:bg-blue-900/30' },
{ label: 'Default (Gray)', value: 'text-gray-800 dark:text-gray-200', bg: 'bg-gray-100 dark:bg-gray-800' },
];

const presets = [
{ label: 'CNC マシニングセンタ', base: 'ControlTerminal', badge: 'MillingMachine', badge2: 'Warning', color:
colorOptions[0].value },
{ label: 'NC旋盤 操作盤', base: 'FATerminal', badge: 'NCLathe', badge2: 'Factory', color: colorOptions[1].value },
{ label: '3Dプリントサーバ', base: 'MiniPC', badge: 'Printer3D', badge2: 'Lab', color: colorOptions[2].value },
{ label: 'ロボットセル制御', base: 'Industrial', badge: 'Manipulator', badge2: 'App', color: colorOptions[0].value },
{ label: '監視・チャート用縦画面', base: 'DualMonitorVerticalPC', badge: 'App', badge2: 'Office', color: colorOptions[2].value },
{ label: 'データ解析 (ポケコン)', base: 'PocketComputer', badge: 'Student', badge2: 'Scroll', color: colorOptions[1].value },
];

return (
<div
    className="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-8 font-sans transition-colors">
    <div className="max-w-5xl mx-auto space-y-12">

        <header className="border-b border-gray-200 dark:border-gray-800 pb-6">
            <h1 className="text-3xl font-bold tracking-tight">Remote Desktop Custom Cursors</h1>
        ...
        <section className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-semibold mb-6 flex items-center">
                <span
                    className="bg-blue-500 text-white w-8 h-8 rounded-full inline-flex items-center justify-center mr-3 text-sm">1</span>
                Interactive Builder
            </h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* Base Selection */}
                    <div>
                        <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Base Form
                            (Hardware/Location)</label>
                        <div className="flex flex-wrap gap-2">
                            {Object.keys(Bases).map(baseKey => (
                            <button key={baseKey} onClick={()=> setSelectedBase(baseKey)}
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors border ${
                                selectedBase === baseKey
                                ? 'bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-900/40 dark:border-blue-700
                                dark:text-blue-300'
                                : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50 dark:bg-gray-800
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Primary
                                Badge (Bottom Right)</label>
                            <select value={selectedBadge} onChange={(e)=> setSelectedBadge(e.target.value)}
                                ...
                                <option key={badgeKey} value={badgeKey}>{badgeKey}</option>
                                ))}
                            </select>
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Status
                                Badge (Top Left)</label>
                            <select value={selectedBadge2} onChange={(e)=> setSelectedBadge2(e.target.value)}
                                className="w-full p-2.5 bg-white border border-gray-300 rounded-lg shadow-sm
                                focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600
                                dark:text-white"
                                >
                    ...
                    {/* Color Selection */}
                    <div>
                        <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Environment
                            Color</label>
                        <div className="flex gap-3">
                            {colorOptions.map((opt, idx) => (
                            <button key={idx} onClick={()=> setSelectedColor(opt.value)}
                                title={opt.label}
                                className={`w-10 h-10 rounded-full border-2 transition-all flex items-center
                                justify-center ${opt.bg} ${
                                selectedColor === opt.value ? 'border-gray-900 dark:border-white scale-110 shadow-md' :
                                'border-transparent opacity-70 hover:opacity-100'
                                }`}
                                >
                                {/* Inner dot reflecting stroke color approximation */}
                                <div className={`w-3 h-3 rounded-full ${opt.value.split(' ')[0].replace(' text-', 'bg-'
                                    )}`}></div>
                            </button>
                            ))}
                        </div>
                ...
                {/* Preview Area */}
                <div
                    className="flex flex-col items-center justify-center bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-8 shadow-inner">
                    <span
                        className="text-sm text-gray-400 dark:text-gray-500 mb-6 uppercase tracking-widest font-semibold">Preview
                        (64px)</span>

                    {/* The Composite Component */}
                    <div className="relative group cursor-crosshair">
                        <CompositeCursor baseName={selectedBase} badgeName={selectedBadge} badge2Name={selectedBadge2}
                            colorClass={selectedColor} size={96} />

                        {/* Hotspot indicator overlay (only visible on hover for demonstration) */}
                        <div
                            className="absolute top-0 left-0 w-3 h-3 bg-red-500 rounded-full -translate-x-1.5 -translate-y-1.5 opacity-0 group-hover:opacity-50 pointer-events-none animate-pulse">
                        </div>
                    </div>
        ...
        <section>
            <h2 className="text-xl font-semibold mb-6 flex items-center">
                <span
                    className="bg-blue-500 text-white w-8 h-8 rounded-full inline-flex items-center justify-center mr-3 text-sm">2</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div key={idx}
                    ...
                    <div className="mb-4 h-16 flex items-center justify-center">
                        <CompositeCursor baseName={preset.base} badgeName={preset.badge} badge2Name={preset.badge2}
                            colorClass={preset.color} size={48} />
                    </div>
                    <h3 className="text-sm font-medium text-center">{preset.label}</h3>
        ...
        <section className="pt-8 border-t border-gray-200 dark:border-gray-800">
            <h2 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300">Raw Components Catalog</h2>

            <div className="mb-6">
                <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-3">Base Forms</h3>
                <div className="flex flex-wrap gap-4">
                    {Object.entries(Bases).map(([name, Component]) => (
                    <div key={name}
                        ...
                        <Component className="w-8 h-8 text-gray-800 dark:text-gray-200 mb-2" />
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-4">
                    {Object.entries(Badges).map(([name, Component]) => (
                    <div key={name}
                        className="flex flex-col items-center bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                        <Component className="w-6 h-6 text-gray-800 dark:text-gray-200 mb-2" />
                        <span className="text-xs text-gray-500">{name}</span>
                    </div>
                    ))}
                </div>
