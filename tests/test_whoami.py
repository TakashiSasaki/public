...
from get_a_grip.tools import whoami
...
class TestWhoami(unittest.TestCase):
    ...
    def test_get_user_principal_name_success(self):
        ...
        with patch('get_a_grip.tools.whoami.sys.platform', 'win32'), \
             patch('get_a_grip.tools.whoami.ctypes') as mock_ctypes:
    ...
    def test_get_user_principal_name_on_linux(self):
        with patch('get_a_grip.tools.whoami.sys.platform', 'linux'):
            result = whoami.get_user_principal_name()
            self.assertIsNone(result)
