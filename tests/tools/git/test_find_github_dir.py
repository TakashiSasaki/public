...
from get_a_grip.core.git.find_github_dir import (
...
def test_get_git_info_gitpython_success():
    with patch('get_a_grip.core.git.find_github_dir.git') as mock_git:
        mock_repo = MagicMock()
...
def test_get_git_info_gitpython_missing():
    with patch('get_a_grip.core.git.find_github_dir.git', None):
        result = get_git_info_gitpython("/path/to/repo")
...
def test_get_git_info_gitpython_ignores_git_env(monkeypatch):
    ...
    with patch("get_a_grip.core.git.find_github_dir.git", FakeGit):
        result = get_git_info_gitpython("/path/to/repo")
...
def test_get_git_info_pygit2_success():
    with patch('get_a_grip.core.git.find_github_dir.pygit2') as mock_pygit2:
        mock_repo = MagicMock()
...
def test_get_git_info_pygit2_missing():
    with patch('get_a_grip.core.git.find_github_dir.pygit2', None):
        result = get_git_info_pygit2("/path/to/repo")
        assert "not installed" in result
...
def test_get_git_info_pygit2_ignores_git_env(monkeypatch):
    ...
    with patch("get_a_grip.core.git.find_github_dir.pygit2", FakePygit2):
        result = get_git_info_pygit2("/path/to/repo")
...
def test_get_git_info_dulwich_success():
    with patch('get_a_grip.core.git.find_github_dir.dulwich') as mock_dulwich:
        mock_repo = MagicMock()
        # Mock HEAD read
...
def test_get_git_info_dulwich_missing():
    with patch('get_a_grip.core.git.find_github_dir.dulwich', None):
        result = get_git_info_dulwich("/path/to/repo")
        assert "not installed" in result
...
def test_get_git_info_dulwich_ignores_git_env(monkeypatch):
    ...
    with patch("get_a_grip.core.git.find_github_dir.dulwich", FakeDulwich):
        result = get_git_info_dulwich("/path/to/repo")
...
def test_get_git_info_dispatch():
    with patch('get_a_grip.core.git.find_github_dir.get_git_info_gitpython') as mock_gp:
        get_git_info("/p", "gitpython")
        mock_gp.assert_called_once()
        
    with patch('get_a_grip.core.git.find_github_dir.get_git_info_pygit2') as mock_pg:
...
@patch('get_a_grip.core.git.find_github_dir.scan_by_ipc')
def test_find_github_dir_real_fs(mock_scan, tmp_path, capsys):
    ...
    from get_a_grip.core.git.find_github_dir import print_github_repos
    with patch('get_a_grip.core.git.find_github_dir.get_git_info_gitpython', side_effect=get_info_side_effect):
        # Also ensure git is mocked as present so the check passes
        with patch('get_a_grip.core.git.find_github_dir.git'):
...
def test_find_github_dir_no_results(capsys):
    from get_a_grip.core.git.find_github_dir import print_github_repos
    with patch('get_a_grip.core.git.find_github_dir.scan_by_ipc', return_value={"dirs": []}):
...
def test_find_github_dir_scan_exception():
    with patch('get_a_grip.core.git.find_github_dir.scan_by_ipc', side_effect=RuntimeError("ipc-fail")):
        data = find_github_dir(backend="gitpython")
